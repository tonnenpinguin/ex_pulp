defmodule ExPulp.QuadraticSolveTest do
  ...
  test "simple QP: minimize x^2 + y^2 s.t. x + y >= 1" do
    problem =
      ExPulp.model "qp_simple", :minimize do
        x = var(low: 0, high: 10)
        y = var(low: 0, high: 10)
  ...
  test "QP with linear + quadratic objective" do
    problem =
      ExPulp.model "qp_mixed", :minimize do
        x = var(low: 0, high: 10)
        y = var(low: 0, high: 10)
  ...
  test "QP with cross terms" do
    problem =
      ExPulp.model "qp_cross", :minimize do
        x = var(low: 0, high: 10)
        y = var(low: 0, high: 10)
  ...
  test "CBC rejects quadratic problems" do
    problem =
      ExPulp.model "qp_cbc", :minimize do
        x = var(low: 0, high: 10)
        minimize x * x
        subject_to x >= 1
      end
  ...
  test "DSL: (x + y)^2 distributes correctly" do
    problem =
      ExPulp.model "qp_square", :minimize do
        x = var(low: 0, high: 10)
        y = var(low: 0, high: 10)
  ...
  test "QP with scaled quadratic: minimize 0.5*x^2" do
    problem =
      ExPulp.model "qp_scaled", :minimize do
