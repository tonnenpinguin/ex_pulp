defmodule ExPulp.ModelReturnTest do
  ...
  test "model returns just problem when last expr is DSL form" do
    result = ExPulp.model "plain", :minimize do
      x = var(low: 0, high: 10)
      minimize x
      subject_to x >= 3
    end
  ...
  test "model returns {problem, map} when last expr is a map" do
    {problem, vars} = ExPulp.model "with_vars", :minimize do
      x = var(low: 0, high: 10)
      y = var(low: 0, high: 10)
      minimize x + y
      subject_to x + y >= 5
  ...
  test "model returns {problem, tuple} when last expr is a tuple" do
    {problem, {p1, p2}} = ExPulp.model "tuple_return", :minimize do
      x = var(low: 0)
      y = var(low: 0)
      minimize x + y
      subject_to x + y >= 1
  ...
  test "returned vars can be used with Result.evaluate" do
    {problem, vars} = ExPulp.model "eval_vars", :minimize do
      x = var(low: 0, high: 10)
      y = var(low: 0, high: 10)
      minimize x + y
      subject_to x + y >= 5
  ...
  test "returned indexed vars for result extraction" do
    ...
    {problem, vars} = ExPulp.model "indexed_return", :minimize do
  ...
  test "returned multi-dim vars for EV charging pattern" do
    ...
    {problem, vars} = ExPulp.model "ev_return", :minimize do
